/**
 * AppInstall — optional POPUP presentation (tenant appInstallModes includes 'popup';
 * off by default). The persistent install UI is the footer banner (FooterAppInstall)
 * + theme buttons — all via the same useGetApp() engine. No fixed/sticky banner here.
 *
 * The popup renders through a portal to <body> at a very high z-index so it always
 * sits above site chrome (headers, loaders, bottom navs) and can't be trapped inside
...
export function AppInstall() {
  ...
  const popup = popupOpen && app.available ? (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Get the app"
      onClick={dismissPopup}
      style={{ position: 'fixed', inset: 0, zIndex: Z_TOP, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,.55)', padding: 'env(safe-area-inset-top) 12px calc(16px + env(safe-area-inset-bottom))' }}
    >
      <div
